...
def inspect_dataset_grains(imgs, masks, res_props=None,
    ...
    for k in range(len(masks)):
        ...
        plt.subplot(int(ceil(len(masks) / 4)), 4, k + 1)
        all_grains_plot(io.imread(str((masks[k]))), elements, props=res_props[k], image=io.imread(str(imgs[k])),
                        title=m_id)
        plt.tight_layout()
    return fig
...
def save_pred_overlays(imgs, preds, save=True, show_n=False, mute=False, tar_dir='', show=False):
    if mute == False:
        for img, pred in tqdm(zip(imgs, preds), desc='Saving images with masks', unit=' images', position=0,
                              leave=True):
    ...
    for _idx in range(len(props)):
        ...
        if 'masks_outline' in elements:
            ...
            for contour in contours:
                plt.plot(contour[:, 1] - (padding_size - .5) + minx, contour[:, 0] - (padding_size - .5) + miny, '-c',
                         linewidth=1.5)
        if 'convex_hull' in elements:
            ...
            for contour in contours:
                plt.plot(contour[:, 1] - (padding_size - .5) + minx, contour[:, 0] - (padding_size - .5) + miny, '-m',
                         linewidth=1.5)
        ...
        if 'fit_b' in elements and b_coords:
            if not b_coords[_idx]:
                ...
                x1 = [b_coords[_idx][0][1] - (padding_size - .5) + minx,
                      b_coords[_idx][1][1] - (padding_size - .5) + minx]
                y1 = [b_coords[_idx][0][0] - (padding_size - .5) + miny,
                      b_coords[_idx][1][0] - (padding_size - .5) + miny]
...
def single_grain_plot(mask, elements, props=None, image=None, fit_res=None,
    ...
    if 'mask' in elements:
        plt.imshow(np.ma.masked_where(mask == 0, mask),
                   extent=[minx - padding_size, maxx + padding_size, maxy + padding_size, miny - padding_size],
                   alpha=.5)
    if 'masks_outline' in elements:
...
def plot_gsd(gsd, color='c', perc_range=np.arange(0.01, 1.01, 0.01), length_max=300, gsd_id=None, title=None,
             label_axes=False, lw=.75, orientation='vertical', units='px', alpha=1):
...
def plot_gsd_uncert(uncert_res, perc_range=np.arange(0.01, 1.01, 0.01), color='k', uncert_area=True,
                    uncert_bounds=False, uncert_median=False, orientation='vertical'):
